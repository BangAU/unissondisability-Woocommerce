...
namespace SW_WAPF_PRO\Includes\Controllers {
    ...
    class Product_Controller
    {
        ...
        public function __construct()
        {
            ...
            add_action('woocommerce_before_calculate_totals',               array($this, 'add_prices_to_cart_item'),2000,1);
		    ...
		    for($i=0; $i < count($cart_data_clones); $i++) {
			    ...
			    if ( $clone_fingerprint !== $fingerprint ) {
				    ...
				    if ( ! $is_in_cart ) {
					    ...
					    add_action('woocommerce_add_to_cart', array($this, 'split_cart_items_by_quantity'),10,6);
					    add_action('woocommerce_add_cart_item_data', array($this, 'add_fields_to_cart_item'),10,4);
				    }
            ...
            if(
	            ...
	            foreach($cart_item['wapf'] as $field) {
                    ...
                    if(Enumerable::from($field['values'])->any(function($x) use($cart_item){
	                    ...
	                    $item_data[] = array(
		                    'key'   => $field['label'],
		                    'value' => Helper::values_to_string($field,isset($cart_item['wapf_item_price']) ? $cart_item['wapf_item_price']['options']:[])
	                    );
                    }
	    ...
	    public function add_meta_to_order_item($item, $cart_item_key, $values, $order) {
		    ...
		    for($i=0;$i<count($values['wapf']);$i++) {
			    ...
			    if(Enumerable::from($field['values'])->any(function($x){ return isset($x['label']) && strlen($x['label']) > 0; })) {
			    	...
			    	$vals = Helper::values_to_string($field,isset($values['wapf_item_price']['options'])? $values['wapf_item_price']['options'] : []);
