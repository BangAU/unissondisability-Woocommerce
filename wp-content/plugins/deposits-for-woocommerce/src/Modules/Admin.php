...
class Admin {
	...
	public function __construct() {
		...
		add_action( 'all_admin_notices', array( $this, 'update_notice_text' ) );
		add_action( 'csf_options_after', array( $this, 'update_notice_option' ) );
		add_action( 'admin_notices', [$this, 'baynaReview'] );
		add_action( 'admin_init', [$this, 'urlParamCheck'] );
		add_action( 'admin_init', array( 'PAnD', 'init' ) );
		add_filter( 'admin_notices', [$this, 'updateNoticeSucessful'], 10 );
		add_filter( 'admin_notices', [$this, 'updateNoticeDB'], 10 );

		add_action( 'init', [$this, 'run_update'], 10 );

		add_action( 'admin_init', [$this, 'paramCheck'], 10 );
	...
	function update_notice_option() {
		echo '<a class="cit-admin-pro-notice" href="https://www.codeixer.com/bayna-upgrade-discount" target="_blank"><p><strong>SPECIAL DISCOUNT</strong> - Save upto 50% Now on Bayna premium version</p> <span>I\'m interested</span></a>';
	}
	/**
	 * Run code on the bayna page's
	 */
	function update_notice_text() {
		$currentScreen = get_current_screen();

		if ( $currentScreen->id === "edit-shop_deposit" ) {
			?>
        <div class="cit-upgrade-sticky">
			<div class="fw-row">
				...
				<div class="fw-col-md-6 buttons-area">
					...
					<a href="https://www.codeixer.com/deposits-payment-plugin-for-woocommerce/" target="_blank" class="cit-button-1"><span class="dashicons dashicons dashicons-upload"></span>Upgrade to PRO</a>
	...
	public function adminScripts() {
		...
		wp_enqueue_script( 'dfwc-admin', CIDW_DEPOSITS_ASSETS . '/js/admin.js', ['jquery'], CIDW_DEPOSITS_VERSION, true );
	}
	/**
	 * check params
	 *
	...
	public function paramCheck() {
		if ( isset( $_GET['bayna_run_update'] ) && $_GET['bayna_run_update'] == 1 ) {
	...
	public function updateNoticeDB() {
        ...
        <div class="notice notice-info ci-notice-error">
            ...
            <?php if ( get_option( 'bayna_run_update' ) == 1 ) {
	...
	public function baynaReview() {
        ...
        <p class="dfwc-message-actions">
            <a style="margin-right:8px;" href="https://wordpress.org/support/plugin/deposits-for-woocommerce/reviews/?filter=5#new-post" target="_blank" class="button button-link">Ok, you deserve it</a>
            <a style="margin-right:8px;" href="<?php echo esc_url( add_query_arg( $temp_dismiss ) ); ?>"  class="button button-link">Nope, maybe later</a>
            <a href="<?php echo wp_nonce_url( add_query_arg( $dismiss_parm ) ); ?>" class="button-link button">I already did</a>
        </p>
        </div>
        <?php }
	}
	...
	public function urlParamCheck() {
		if ( isset( $_GET['dfwc-review-dismiss'] ) && $_GET['dfwc-review-dismiss'] == 1 ) {
			update_option( 'dfwc_plugin_review', 1 );
		}
		if ( isset( $_GET['dfwc-review-dismiss-temp'] ) && $_GET['dfwc-review-dismiss-temp'] == 1 ) {
