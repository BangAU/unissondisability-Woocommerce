...
<table border="0" cellpadding="20" cellspacing="0" style="width:100%; margin-bottom:15px">
    ...
    <tr>
        ...
        <th class="td" style="color: #636363; border: 1px solid #e5e5e5; vertical-align: middle; padding: 12px;"><?php esc_html_e( 'Actions', 'deposits-for-woocommerce' );?> </th>
...
foreach ( $depositList as $key => $deposit ) {
    $depositOrder = new \Deposits_WooCommerce\ShopDeposit( $deposit->ID );?>

        <tr class="order_item">

            <td class="td" style="color: #636363; border: 1px solid #e5e5e5; vertical-align: middle; padding: 12px;">
              <?php echo '<strong>#' . get_post_meta( $deposit->ID, '_deposit_id', true ) . '</strong>'; ?>
